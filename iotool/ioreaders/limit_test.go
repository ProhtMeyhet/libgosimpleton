...
func TestLimitReader(t *testing.T) {
	...
	for {
		...
		if e != nil {
			if e != io.EOF {
				t.Fatalf("unexpected error while reading: %v", e)
			}

			break
		}

		total = append(total, into[:read]...)
	}

	if !bytes.Equal(total, getLimitedData()) {
		t.Errorf("read read: \n\n%s\n\nexpected:\n\n%s", total, getLimitedData())
	}
}
